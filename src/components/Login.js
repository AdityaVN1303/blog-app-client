...
const Login = () => {
  const [email, setEmail] = useState("adityanagare09@gmail.com");
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch(
        "https://blog-app-server-red.vercel.app/login",
        {
          method: "POST",
  ...
  return (
    <div className="login h-screen w-screen bg-slate-900 text-white flex justify-center items-center">
      <div className="card p-5 rounded-md flex flex-col space-y-5 bg-blue-900">
        ...
        <button
          onClick={handleLogin}
          className="px-5 rounded-md py-2 bg-purple-950 hover:bg-purple-400 text-white text-lg"
        >
          Sign In
        </button>
